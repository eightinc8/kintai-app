...
function rowToReport(row: Record<string, string>): DailyReport {
  return {
    ...
    amazonCount: Number(row.amazon_count) || 0,
    rakutenCount: Number(row.rakuten_count) || 0,
...
function reportToRow(r: DailyReport): Record<string, string> {
  return {
    ...
    amazon_count: String(r.amazonCount || 0),
    rakuten_count: String(r.rakutenCount || 0),
...
export async function submitReport(
  ...
  const report: DailyReport = {
    ...
    rakutenCount: data.rakutenCount ?? 0,
...
export async function updateReportById(
  ...
  const updated: DailyReport = {
    ...
    amazonCount: data.amazonCount ?? existing.amazonCount,
    rakutenCount: data.rakutenCount ?? existing.rakutenCount,
